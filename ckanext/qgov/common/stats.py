...
class Stats(object):
    ...
    def resource_count(cls):
        ...
        query = select([func.count(resource.c.id)]).\
            where(and_(resource.c.state == 'active', package.c.state == 'active', resource_group.c.id == resource.c.resource_group_id, package.c.id == resource_group.c.package_id))
    ...
    def resource_report(cls):
        ...
        query = select([group.c.title, package.c.title, resource.c.name, resource.c.url, resource.c.created, resource.c.last_modified, resource.c.format, resource.c.webstore_url, resource.c.resource_type]).\
            where(and_(resource.c.state == 'active', resource_group.c.id == resource.c.resource_group_id, package.c.id == resource_group.c.package_id, group.c.id == package.c.owner_org))

        res_report = model.Session.execute(query).fetchall()
        return res_report

    @classmethod
    def resource_org_count(org_id):
        resource = table('resource')
        package = table('package')
        resource_group = table('resource_group')

        query = select([func.count(resource.c.id)]).\
         where(and_(resource.c.state == 'active', package.c.state == 'active', resource_group.c.id == resource.c.resource_group_id, package.c.id == resource_group.c.package_id, org_id == package.c.owner_org))
